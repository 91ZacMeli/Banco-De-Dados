...
public class AnunciosPersistence {
    ...
    public List<Anuncio> lista() {
        ...
        try {
            PreparedStatement ps = cnx.prepareStatement("select * from anuncios");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                anuncioList.add(new Anuncio(rs.getString(1), rs.getString(2), rs.getBigDecimal(3), rs.getString(4), rs.getInt(5)));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void insere(Anuncio anuncio) {
        try {
            ...
            PreparedStatement ps = cnx.prepareStatement("insert into anuncios (codigo, titulo, preco, dataDoAnuncio,numeroDeVendas) " + " values (?,?,?,?,?)");
            ps.setString(1, anuncio.getCodigo());
            ps.setString(2, anuncio.getTitulo());
            ps.setBigDecimal(3, anuncio.getPreco());
            ps.setString(4,  anuncio.getDataDoAnuncio());
    ...
    public void atualiza(Anuncio anuncio) {
        try {
            PreparedStatement ps = cnx.prepareStatement("update anuncios set titulo = ?, preco =  ?, dataDoAnuncio = ?,numeroDeVendas = ? where  codigo = ?");
            ps.setString(1, anuncio.getTitulo());
            ps.setBigDecimal(2, anuncio.getPreco());
            ps.setString(3, anuncio.getDataDoAnuncio());
    ...
    public void exclui(String codigo){
        try {
            PreparedStatement ps = cnx.prepareStatement("delete from anuncios where codigo = ?");
